...
<c:forEach items = "${reviewer_id}" var = "reviewer">
    ...
    <sql:query var = "r_assign_verify">
        select * from reviewer_assignment where reviewer_id = ? and cfp_code = ? and tracking_code = ?
            ...
            <c:choose>
                ...
                <c:otherwise>
                    ...
                    <sql:query var = "lookup_value" maxRows = "1">
                        select assignment_id from reviewer_assignment order by assignment_id desc
                    </sql:query>

                    <c:set var = "assignment_id2" value = "${lookup_value.rows[0].assignment_id + 1}" scope = "page"/>
                    ...
                    <c:if test = "${param.report_assigned=='R'}">
                        <c:set var = "report" value = "1" scope = "page"/>
                    </c:if>

                    <c:set var = "proposal" value = "0" scope = "page"/>

                    <c:if test = "${param.proposal_assigned=='P'}">
                        <c:set var = "proposal" value = "1" scope = "page"/>
                    </c:if>

                    <sql:update>
                        insert into reviewer_assignment ( assignment_id, reviewer_id, cfp_code, tracking_code, report,
                        proposal ) values ( ?, ?, ?, ?, ?, ? )
                    ...
                    <sql:update>
                        update proponent_record set status_id = 4 where tracking_code = ?
                    ...
                    <sql:query var = "reviewers_email">
                        select reviewer_email, reviewer_lastname, reviewer_firstname, reviewer_login,
                        reviewer_password from reviewers where reviewer_id = ?

                        <sql:param value = "${reviewer}"/>
                    </sql:query>

                    <c:set var = "re" value = "${reviewers_email.rows[0]}" scope = "page"/>

                    <c:set var = "reviewer_email" value = "${reviewers_email.rows[0].reviewer_email}" scope = "page"/>
                    ...
                    <sql:update>
                        insert into sent_messages (sent_message_id, tracking_code, letter_id, date_sent, recipient1 )
                        values (?, ?, 99, CURDATE(), ? )
