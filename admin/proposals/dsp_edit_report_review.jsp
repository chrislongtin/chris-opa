...
<sql:query var = "report_appraisal">
    select ra.*, d.doc_title, pr.proposal_title,d.doc_filename from report_appraisal ra, documents d, document_types
    dt, proponent_record pr where ra.reviewer_id = ? and ra.tracking_code = ? and d.doc_id = ra.doc_id and
    d.doc_type_id = dt.doc_type_id and ra.tracking_code = pr.tracking_code order by ra.report_appraisal_id
...
<c:forEach var = "row" items = "${report_appraisal.rows}">
    ...
    <cf:GetPhrase phrase_id = "114" lang_id = "${lang}"/>:</font><b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    ...
    <form action = "<c:url value='index.jsp'>
     <c:param name='fuseaction' value='act_report_edit_review'/>
        ...
        <input type = "hidden" name = "tracking_code" value = "<c:out value="${row.tracking_code}" />">
