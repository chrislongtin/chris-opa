...
<sql:query var = "prop">
    select distinct p.tracking_code from proponent_record p, contractor_assignment ca, cfp_info c where
    ca.contractor_id = ? and ca.tracking_code = p.tracking_code and ca.tracking_code = ? and c.cfp_code = p.cfp_code
    and (ca.end_date >= CURDATE() or ca.end_date is null)
...
<c:if test = "${prop.rowCount == 1}">
    ...
    <c:choose>
		...
		<c:otherwise>
	    <sql:query var="over">
		select * from contractor_timesheets
	      where contractor_id = ? and
	      tracking_code = ? and
		...
		<sql:update>
			insert into contractor_timesheets
		  (status_id, contractor_id, tracking_code, start_date, end_date )
		  values
		  (1, ?, ?, ?, ?)
		  <sql:param value="${sessionScope.ctid}" />
		  <sql:param value="${tc}" />
