...
<sql:query var = "prop">
    select p.tracking_code, p.proposal_title, p.cfp_code from proponent_record p, contractor_assignment ca, cfp_info c
    where ca.contractor_id = ? and ca.tracking_code = p.tracking_code and c.cfp_code = p.cfp_code and (ca.end_date >=
    CURDATE() or ca.end_date is null) order by p.tracking_code desc
...
<c:choose>
    <c:when test = "${prop.rowCount > 0}">
        <form name = "tsc_form" method = "post" action = "index.jsp?fuseaction=c_tsr_docreate">
    ...
    <c:otherwise>
        <cf:GetPhrase phrase_id = "969" lang_id = "${lang}"/>
    </c:otherwise>
</c:choose>
