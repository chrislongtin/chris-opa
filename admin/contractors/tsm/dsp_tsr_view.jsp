...
<sql:query var = "prop">
    select p.proposal_title, t.*, s.ts_status from proponent_record p, contractor_timesheets t, timesheetstatus s
    where t.contractor_id = ? and t.timesheet_id = ? and t.tracking_code = p.tracking_code and t.status_id = s.status_id
...
<c:if test = "${prop.rowCount == 1}">
    ...
    <c:choose>
        <c:when test = "${ts.status_id == 1}">
            <form name = "del_form" method = "post" action = "index.jsp?fuseaction=c_tsr_update">
            ...
            <c:if test = "${tr.rowCount > 0}">
                <form name = "submit_form" method = "post" action = "index.jsp?fuseaction=c_tsr_update">
                ...
                <cf:GetPhrase phrase_id = "849" lang_id = "${lang}"/></h4><c:forEach var = "row" items = "${tr.rows}">
                    <form name = "tr_form" method = "post" action = "index.jsp?fuseaction=c_tsr_update">
            ...
            <form name = "add_tr_form" method = "post" action = "index.jsp?fuseaction=c_tsr_update">
                <cf:GetPhrase phrase_id = "171" lang_id = "${lang}"/>
