    ...
    <c:when test = "${sessionScope.user=='coordinator'}">
        ...
        <table border = "1" cellspacing = "0" cellpadding = "3">
            <tr>
                ...
                <td>
                    <font face = "Arial" size = "-1"><cfoutput query = "worksite_check">
                        <c:if test = "${empty worksite_check.rows[0].host_url}">
                            <li><cf:GetPhrase phrase_id = "211" lang_id = "${lang}"/>
                        </c:if>

                        <c:if test = "${empty worksite_check.rows[0].host_doc_dir}">
                            <li><cf:GetPhrase phrase_id = "212" lang_id = "${lang}"/>
                        </c:if>

                        <c:if test = "${empty worksite_check.rows[0].public_info}">
                            <li><cf:GetPhrase phrase_id = "387" lang_id = "${lang}"/>
                        </c:if>
            ...
            <tr>
                ...
                <td>
                    <font face = "Arial" size = "-1"><c:if test = "${empty initiative_info.rows[0].initiative_name}">
                        <li><cf:GetPhrase phrase_id = "207" lang_id = "${lang}"/>
                    </c:if>

                    <c:if test = "${empty initiative_info.rows[0].background}">
                        <li><cf:GetPhrase phrase_id = "403" lang_id = "${lang}"/>
                    </c:if>
                    ...
                    <c:if test = "${empty initiative_info.rows[0].proposal_format}">
                        <li><cf:GetPhrase phrase_id = "404" lang_id = "${lang}"/>
                    </c:if>

                    <c:if test = "${empty initiative_info.rows[0].copyright}">
                        <li><cf:GetPhrase phrase_id = "405" lang_id = "${lang}"/>
                    </c:if>

                    <c:if test = "${empty initiative_info.rows[0].record_lifecycle}">
                        <li><cf:GetPhrase phrase_id = "406" lang_id = "${lang}"/>
                    </c:if>
                    ...
                    <td>
                        <font face = "Arial" size = "-1"><c:choose>
                            <c:when test = "${empty criteria.rows[0].i_criteria_id}">
                                <cf:GetPhrase phrase_id = "420" lang_id = "${lang}"/>
                            </c:when>

                            <c:otherwise>
                                <cf:GetPhrase phrase_id = "421" lang_id = "${lang}"/>
                            </c:otherwise>
                        </c:choose>
                    </td>
                </tr>
                ...
                <td>
                    <font face = "Arial" size = "-1"><c:choose>
                        <c:when test = "${proponents.rowCount > assignments.rowCount}">
                            <cf:GetPhrase phrase_id = "426" lang_id = "${lang}"/>
                        </c:when>
                ...
                <td>
                    <font face = "Arial" size = "-1"><c:choose>
                        <c:when test = "${letters.rowCount==0}">
                            <cf:GetPhrase phrase_id = "182" lang_id = "${lang}"/>
                        </c:when>
            ...
            <c:otherwise>
                ...
                <ul>
                    <c:forEach items = "${proposal_info.rows}" var = "row">
                        <li><a STYLE="text-decoration: underline"  href = "index.jsp?fuseaction=proposal_review&tracking_code=<c:out value="${proposal_info.rows[0].tracking_code}" />">
                ...
                <sql:query var = "edit_reviews">
                    select ra.assignment_id, ra.tracking_code, p.proposal_title from reviewer_assignment ra, cfp_info
                    c, proponent_record p where ra.reviewer_id = ? and ra.proposal_review_completed = 1 and
                    ra.proposal = 1 and ra.cfp_code = c.cfp_code and c.cfp_proposal_review_deadline >= CURDATE() and
