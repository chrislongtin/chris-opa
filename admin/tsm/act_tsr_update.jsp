...
<sql:query var = "timesheet">
    select p.proposal_title, t.* from proponent_record p, timesheets t where t.reviewer_id = ? and t.timesheet_id = ?
    and t.tracking_code = p.tracking_code
...
<c:choose>
    <c:when test = "${act == 'delete'}">
        <c:choose>
            <c:when test = "${ts.status_id == 1}">
                ...
                <p>
                <a STYLE="text-decoration: underline"  href = "index.jsp?fuseaction=tsr_update&act=dodelete&ts=<c:out value="${ts_id}" />">[

                <cf:GetPhrase phrase_id = "542" lang_id = "${lang}"/> ]</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                <a STYLE="text-decoration: underline"  href = "index.jsp?fuseaction=tsr_view&ts=<c:out value="${ts_id}" />">[
    ...
    <c:when test = "${act == 'dodelete'}">
        <c:choose>
            <c:when test = "${ts.status_id == 1}">
                <sql:update>
                    delete from timerecords where timesheet_id = ?
    ...
    <c:when test = "${act == 'submit'}">
        <c:choose>
            <c:when test = "${ts.status_id == 1}">
                ...
                <p>
                <a STYLE="text-decoration: underline"  href = "index.jsp?fuseaction=tsr_update&act=dosubmit&ts=<c:out value="${ts_id}" />">[
    ...
    <c:when test = "${act == 'dosubmit'}">
        <c:choose>
            <c:when test = "${ts.status_id == 1}">
                <sql:update>
                    update timesheets set status_id = 2 where timesheet_id = ?
...
<c:set var="comment"><c:out value="${param.comment}" default=""/></c:set>  
  ...
  <c:import url="tsm/dsp_tsr_view.jsp?ts=${ts_id}" />
