...
<c:if test = "${(sessionScope.user == 'coordinator') || (sessionScope.rcoord == 1)}">
    <sql:query var = "ts">
        ...
        timesheets.timesheet_id = ? and timesheets.status_id = timesheetstatus.status_id and timesheets.tracking_code
        = proponent_record.tracking_code and timesheets.reviewer_id = reviewers.reviewer_id and ( timesheets.status_id
        = 2 or timesheets.status_id = 3 )
    ...
    <c:if test = "${ts.rowCount == 1}">
        ...
        <c:out value = "${ts.reviewer_firstname} ${ts.reviewer_lastname}"/>
        ...
        <c:if test = "${(ts.status_id == 2) && (sessionScope.user == 'coordinator')}">
            <form name = "app_form" method = "post" action = "index.jsp?fuseaction=tsc_update">
                    ...
                    <td bgcolor = "#EEEEEE"                   align = "center" valign = "middle""> <font
                        ...
                        <c:choose>
                            <c:when test = "${(ts.status_id == 2) && (sessionScope.user == 'coordinator')}">
                                <form name = "app_h_form" method = "post" action = "index.jsp?fuseaction=tsc_update">
                                    <input type = "text"
                                           name = "approved"
                                           size = "2"
