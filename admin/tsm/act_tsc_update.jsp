...
<c:if test = "${sessionScope.user == 'coordinator'}">
    ...
    <c:if test = "${act == 'approve_h'}">
        <sql:query var = "timerecord">
            select * from timerecords where record_id = ?
    ...
    <c:choose>
        <c:when test = "${act == 'approve_ts'}">
            <c:choose>
                <c:when test = "${ts.status_id == 2}">
                    ...
                    <p>
                    <a STYLE="text-decoration: underline"  href = "index.jsp?fuseaction=tsc_update&act=doapprove_ts&ts=<c:out value="${ts_id}" />">[

                    <cf:GetPhrase phrase_id = "542" lang_id = "${lang}"/> ]</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    <a STYLE="text-decoration: underline"  href = "index.jsp?fuseaction=tsc_view&ts=<c:out value="${ts_id}" />">[
        ...
        <c:when test = "${act == 'doapprove_ts'}">
            <c:choose>
                <c:when test = "${ts.status_id == 2}">
                    <sql:update>
                        update timesheets set status_id = 3 where timesheet_id = ?

                        <sql:param value = "${ts_id}"/>
                    </sql:update>

                    <sql:query var = "r">
                        select reviewers.reviewer_lastname, reviewers.reviewer_firstname, reviewers.reviewer_email
                        from reviewers, timesheets where timesheets.timesheet_id = ? and timesheets.reviewer_id =
                    ...
                    <mt:mail session = "java:/comp/env/mail/session">
                        ...
                        <mt:setrecipient type = "to">
                            <c:out value = "${r.rows[0].reviewer_email}"/>
                        </mt:setrecipient>

                        <mt:subject>
                            <cf:GetPhrase phrase_id = "841" lang_id = "${lang}"/>
                        ...
                        <mt:message>
                            ...
                            <c:out value = "${r.rows[0].reviewer_firstname} ${r.rows[0].reviewer_lastname}"/>
                    ...
                    <c:import url = "tsm/dsp_tsc_main.jsp?tracking_code=${ts.tracking_code}"/>
                </c:when>
        ...
        <c:when test = "${act == 'approve_h'}">
            <c:choose>
                <c:when test = "${ts.status_id == 2}">
                    <sql:update>
                        update timerecords set hours_approved = ? where record_id = ?
                    ...
                    <c:import url = "tsm/dsp_tsc_view.jsp?ts=${ts.timesheet_id}"/>
                </c:when>
