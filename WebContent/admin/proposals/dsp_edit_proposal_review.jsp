...
<sql:query var = "cfp_appraisal_info">
    select pa.*, cfp_criteria_name, cfp_criteria_weight, cfp_low_rank,
    cfp_high_rank from proposal_appraisal pa, cfp_criteria c where
    pa.reviewer_id = ? and pa.tracking_code = ? and pa.i_criteria_id = 0 and
    pa.cfp_criteria_id = c.cfp_criteria_id
...
<sql:query var = "i_appraisal_info">
    select pa.*, i_criteria_name, i_criteria_weight, i_low_rank, i_high_rank
    from proposal_appraisal pa, initiative_criteria ic where pa.reviewer_id =
    ? and pa.tracking_code = ? and pa.cfp_criteria_id = 0 and pa.i_criteria_id
    = ic.i_criteria_id
...
<fmt:message key = "595" />:</b><c:choose>
    ...
    <c:otherwise>
        <a STYLE = "text-decoration: underline"
           href = "index.jsp?fuseaction=proposal_docs_list&tracking_code=<c:out value="${tracking_code}" />&source=review_edit">
...
<table border = "0" cellspacing = "0" cellpadding = "2">
    <form action = "index.jsp?fuseaction=act_proposal_edit_review"
          method = "post">
        ...
        value = "<c:out value="${tracking_code}" />"><c:if test = "${use_init_crit==1}">
            ...
            <c:if test = "${rankings > 1}">
                <input type = "radio" name = "rank<c:out value="${rank}" />"
                       value = "2"    <c:if
            ...
            name = "cfp_criteria_id" value = "0">
            <input type = "hidden" name = "i_criteria_weight"
            value = "<c:out value="${row.i_criteria_weight}" />"> <input type = "hidden"
            name = "cfp_criteria_weight" value = "0"> <input type = "hidden"
            name = "appraisal_id"
            value = "<c:out value="${row.appraisal_id}" />">
            ...
            <c:forEach items = "${cfp_appraisal_info.rows}" var = "row">
                ...
                value = "<c:out value="${row.cfp_criteria_weight}" />"> <input type = "hidden" name = "i_criteria_weight" value = "0">
            ...
            value = "<c:out value="${maxrank}" />"><sql:query var = "deadline">
                select cfp_proposal_review_deadline from cfp_info where
                cfp_code = ?
