...
<sql:query var = "cfp_current_list">
    select cfp_title, cfp_code from cfp_info where cfp_deadline >= CURDATE() and CURDATE() >= cfp_startdate order by
    cfp_code
</sql:query>
    ...
    <c:otherwise>
        <ul>
            <c:forEach items = "${cfp_current_list.rows}" var = "row">
                <p>
                <li><b><a STYLE="text-decoration: underline"  href = "index.jsp?fuseaction=cfp_info&cfp_code=<c:out value="${row.cfp_code}" />&lang=<c:out value="${lang}" />">(CFP-
