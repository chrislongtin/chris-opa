...
<sql:query var = "doc_info">
    select d.doc_title, p.proponent_leader_firstname, p.proponent_leader_lastname, d.tracking_code, d.doc_filename,
    p.cfp_code from documents d, proponent_record p where d.doc_id = ? and d.tracking_code = p.tracking_code
...
<c:set var = "host_url" value = "${download_dir.rows[0].host_url}" scope = "page"/>

<c:set var = "public_attachments" value = "${download_dir.rows[0].public_attachments}" scope = "page"/>
...
<br>
<a STYLE="text-decoration: underline"  href = "index.jsp?fuseaction=archive_proposal&tracking_code=<c:out value="${di.tracking_code}" />&lang=<c:out value="${lang}" />"><i>

<c:out value = "${di.proponent_leader_firstname} ${di.proponent_leader_lastname}"/></i></a>
<c:if test = "${di.doc_filename!=''}">
    <p>
    <a STYLE="text-decoration: underline"  href = "docs/<c:out value="${di.doc_filename}" />?ois=no">

    <cf:GetPhrase phrase_id = "34" lang_id = "${lang}"/></a> |
    <a STYLE="text-decoration: underline"  href = "docs/<c:out value="${di.doc_filename}" />?ois=yes">
...
<c:set var = "cfp_code" value = "${di.cfp_code}" scope = "page"/><c:choose>
    <c:when test = "${act=='view'}">
        ...
        <a STYLE="text-decoration: underline"  href = "index.jsp?fuseaction=archive_comment&doc_id=<c:out value="${doc_id}" />&cfp_code=<c:out value="${cfp_code}" />&act=add">
        <b>
        ...
        <sql:query var = "qn">
            select * from discussion where doc_id = ? and discuss_parent = 0 order by discuss_date desc
        ...
        <ul>
            <c:forEach items = "${qn.rows}" var = "row">
                ...
                <!--- messages output --->
                <p>
                <li><font size = "3"><a STYLE="text-decoration: underline"  href = "index.jsp?fuseaction=archive_comment&doc_id=<c:out value="${doc_id}" />&lang=<c:out value="${lang}" />&show_msg=yes&id_num=<c:out value="${row.discussion_id}" />">
                ...
                <c:if test = "${row.discuss_replies!=0}">
                    <c:set var = "parent" value = "${row.discussion_id}" scope = "page"/>
        ...
        <c:if test = "${act=='add'}">
            ...
            <c:choose>
                ...
                <c:otherwise>
                    ...
                    <c:set var = "mr" value = "${message_reply.rows[0]}" scope = "page"/>
                    ...
                    <fmt:formatDate value = "${mr.discuss_date}" pattern = "MMM dd, yyyy"/></font>

                    <br>
                    <menu><cf:ParagraphFormat value = "${mr.discuss_message}"/></menu>

                    <hr size = "1">
                    <c:set var = "discuss_level" value = "${discuss_level + 1}" scope = "page"/>
                </c:otherwise>
            </c:choose>
            ...
            <form action = "index.jsp?fuseaction=act_archive_comment&lang=<c:out value="${lang}" />" method = "post"
                  ENCTYPE = "multipart/form-data">
                <input type = "hidden" name = "tracking_code" value = "<c:out value="${di.tracking_code}" />">
                <input type = "hidden" name = "doc_id" value = "<c:out value="${doc_id}" />">
                <input type = "hidden" name = "cfp_code" value = "<c:out value="${di.cfp_code}" />">
